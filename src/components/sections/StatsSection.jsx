...
const StatCard = ({ item, i, inView }) => {
  ...
  return (
    ...
    >
      {/* Card Container */}
      <div className="relative p-10 rounded-[2.5rem] bg-card/50 backdrop-blur-xl border border-border transition-all duration-700 hover:border-primary/20 hover:shadow-[0_0_50px_-12px_rgba(59,130,246,0.3)] overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center" style={{ transform: "translateZ(50px)" }}>
          <span className="text-[100px] font-black text-foreground/[0.02] absolute -top-8 left-1/2 -translate-x-1/2 select-none pointer-events-none transition-transform duration-700 group-hover:scale-125 group-hover:text-primary/10">
            {item.value}
          </span>

          <h3 className="text-4xl md:text-6xl lg:text-7xl font-black text-foreground tracking-tighter transition-all duration-500 group-hover:text-primary group-hover:scale-110 drop-shadow-[0_0_20px_rgba(255,255,255,0.1)]">
            {inView ? <CountUp end={item.value} duration={2.5} /> : "0"}{item.suffix || "+"}
          </h3>

          <p className="mt-6 text-xs font-bold text-muted-foreground uppercase tracking-[0.3em] group-hover:text-foreground transition-colors duration-500 text-center">
            {item.label}
          </p>
...
export const StatsSection = () => {
  ...
  return (
    <div ref={ref} className="bg-background py-20 md:py-24 select-none overflow-hidden">
