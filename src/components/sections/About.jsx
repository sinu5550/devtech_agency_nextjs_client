  ...
  return (
    <div id="about" className="container mx-auto py-20 md:py-32 px-4 select-none">
      <div className="text-center mb-20">
        ...
        <h2 className="text-3xl md:text-5xl lg:text-6xl font-black mb-8 text-white tracking-tighter leading-tight">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-8">
        {features.map((item, index) => {
          return (
            <Card 
              key={index} 
              className="group relative transition-all duration-500 hover:shadow-[0_0_40px_-10px_rgba(59,130,246,0.2)] hover:border-primary/20 border-white/5 bg-white/[0.02] backdrop-blur-xl overflow-hidden"
            >
              {/* Dashboard Elements */}
              ...
              <CardContent className="flex flex-col p-8 md:p-10 h-full">
                ...
                <h4 className="font-black text-2xl mb-4 text-white group-hover:text-primary transition-colors flex items-center gap-2">
                  {item.title}
                  <CheckCircle2 className="w-4 h-4 text-primary opacity-0 group-hover:opacity-100 transition-opacity" />
                </h4>
                
                <p className="text-base font-medium text-slate-400 leading-relaxed group-hover:text-slate-300 transition-colors mb-10">
                  {item.desc}
                </p>
